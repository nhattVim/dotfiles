...
if [ -d "$HOME/.oh-my-zsh" ]; then
    printf "\n%s - Oh My Zsh is already installed. \n" "${NOTE}"
else
    printf "\n%s - Download oh-my-zsh ... \n" "${NOTE}"
    if sh -c 'sh -c "$(wget -O- https://install.ohmyz.sh)"'; then
        printf "\n%s - Download oh-my-zsh successfully \n" "${OK}"
        git clone https://github.com/zsh-users/zsh-autosuggestions ${ZSH_CUSTOM:-~/.oh-my-zsh/custom}/plugins/zsh-autosuggestions
    else
        printf "\n%s - Failed to download oh-my-zsh \n" "${ERROR}"
    fi
fi
