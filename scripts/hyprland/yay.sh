...
if [ -n "$ISAUR" ]; then
    ...
    note "AUR helper NOT found, installing yay from AUR..."
    ...
    while [ $attempt -le $MAX_RETRIES ]; do
        temp_dir=$(mktemp -d)
        note "Attempt $attempt: Cloning yay..."
        if git clone https://aur.archlinux.org/yay.git "$temp_dir" && cd "$temp_dir"; then
            if makepkg -si --noconfirm; then
                ok "Successfully installed yay!"
                rm -rf "$temp_dir"
                break
            else
                err "Failed to build/install yay (Attempt $attempt)"
            fi
        else
