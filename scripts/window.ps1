...
$scoopPackages = @(
    "fzf", "lazygit", "tere", "git", "gcc", "nvm", "pnpm", "openjdk", "python",
    "make", "oh-my-posh", "lsd", "winfetch", "fastfetch", "ripgrep", "unzip",
    "wget", "gzip", "pwsh", "winrar", "autoclicker", "firefox", "neovim",
    "neovide", "abdownloadmanager", "flow-launcher", "geekuninstaller", "treesize-free"
)
...
function Install-Packages
{
    ...
    foreach ($pkg in $Packages)
    {
        ...
        {
            'scoop'
            { $command = "scoop install $pkg" 
            }
            'winget'
            { $command = "winget install --id=$pkg --silent --accept-package-agreements --accept-source-agreements" 
            }
            default
            { Write-ErrorLog "Unknown package manager: $PackageManager"; return 
            }
        }
...
if (Test-Path $dotfilesTempDir)
{ Remove-Item $dotfilesTempDir -Recurse -Force 
}
...
try
{
    ...
    Start-Process (Join-Path $env:UserProfile "scoop\apps\flow-launcher\current\Flow.Launcher.exe") -ErrorAction Stop    
} catch
{
